...
namespace MessageFlowSmsEmailFunc.Services
{
    public class SmsService
    {
        ...
        {
            _logger.LogInformation("SmsService.SendSmsAsync called with request: {Request}",
                JsonSerializer.Serialize(request, new JsonSerializerOptions { WriteIndented = true }));

            try
            {
                // Get configuration values
                var authorization = _configuration["RestApiAuthorization"];
                var applicationKey = _configuration["RestApiApplicationKey"];
